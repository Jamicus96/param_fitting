...
Reactor::Reactor(FitVar* vDm_21_2, FitVar* vDm_32_2, FitVar* vS_12_2, FitVar* vS_13_2, const std::vector<TH1D*>& Reactor_hists,
    ...
    if (Reactor_names.size() != Norms.size()) {
        std::cout << "ERROR: Reactor_names and Norms should have the same size!" << std::endl;
        exit(1);
    }
...
void Reactor::compute_spec() {
    ...
    if (!(Vars.at(iDm_21_2)->isConstant() && Vars.at(iDm_32_2)->isConstant() && Vars.at(vS_12_2)->isConstant() && Vars.at(iS_13_2)->isConstant() && computed_osc_specs)) {
...
std::vector<double> Reactor::GetOscReactorNorms() {
    std::vector<double> osc_reac_norms;
    double tot_osc_int = model_spec->Integral();
    for (unsigned int i = 0; i < osc_hists.size(); ++i) {
        osc_reac_norms.push_back(tot_osc_int * Vars.at(iNorms.at(i))->val());
    }
    return osc_reac_norms;
}
