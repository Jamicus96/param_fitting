...
class alphaN {
    private:
        ...
        unsigned int iNormPR, iNormC12, iNormES, iEsys, iEsysP;
        TH1D* hist_ProtontR;
        TH1D* hist_C12Scatter;
        TH1D* hist_O16Deex;
        double Integral_hist_PR, Integral_hist_C12, Integral_hist_ES;
        TH1D* model_Proton;
    ...
    public:
        ...
        // Initialisers
        void InitAlphaN(const unsigned int NormPR_idx, const unsigned int NormC12_idx, const unsigned int NormES_idx, const unsigned int E_syst_idx,
               const unsigned int E_syst_proton_idx, TH1D* Hist_ProtontR, TH1D* Hist_C12Scatter, TH1D* Hist_O16Deex) {

            iNormPR = NormPR_idx; iNormC12 = NormC12_idx; iNormES = NormES_idx; iEsys = E_syst_idx; iEsysP = E_syst_proton_idx;
            iMinBin = 1; iMaxBin = Hist_ProtontR->GetXaxis()->GetNbins();

            hist_ProtontR = Hist_ProtontR;
            Integral_hist_PR = hist_ProtontR->Integral(iMinBin, iMaxBin);
            hist_C12Scatter = Hist_C12Scatter;
            Integral_hist_C12 = hist_C12Scatter->Integral(iMinBin, iMaxBin);
        ...
        void InitAlphaN(const std::string NormPR_name, const std::string NormC12_name, const std::string NormES_name, const std::string E_syst_name,
            ...
            InitAlphaN(Vars->findIdx(NormPR_name), Vars->findIdx(NormC12_name), Vars->findIdx(NormES_name), Esysts->findIdx(E_syst_name),
        ...
        void compute_spec() {
            ...
            model_Esys->Reset("ICES");  // empty it before re-computing it
                ...
                std::cout << "[alphaN::compute_spec]: Vars->val(iNormPR) = " << Vars->val(iNormPR) << std::endl;
                std::cout << "[alphaN::compute_spec]: Vars->val(iNormC12) = " << Vars->val(iNormC12) << std::endl;
            ...
            model_Proton->Add(hist_ProtontR, Vars->val(iNormPR) / Integral_hist_PR);
            Esysts->apply_systematics(iEsysP, model_Proton, model_noEsys);

            // Add the non proton-recoil spectra to spectrum
            model_noEsys->Add(hist_C12Scatter, Vars->val(iNormC12) / Integral_hist_C12);
        ...
        void Spectra(std::vector<TH1D*>& hists) {
            ...
            temp_hist->Add(hist_ProtontR, Vars->val(iNormPR) / Integral_hist_PR);
            ...
            temp_hist->Reset("ICES");
            temp_hist->Add(hist_C12Scatter, Vars->val(iNormC12) / Integral_hist_C12);
