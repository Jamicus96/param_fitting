...
class alphaN {
    private:
        ...
        unsigned int iNormPR, iNormGS, iNormES, iEsys, iEsysP;
        TH1D* hist_ProtontR;
    ...
    public:
        ...
        // Initialisers
        void InitAlphaN(const unsigned int NormPR_idx, const unsigned int NormGS_idx, const unsigned int NormES_idx, const unsigned int E_syst_idx,
               const unsigned int E_syst_proton_idx, TH1D* Hist_ProtontR, TH1D* Hist_C12Scatter, TH1D* Hist_O16Deex) {

            iNormPR = NormPR_idx; iNormGS = NormGS_idx; iNormES = NormES_idx; iEsys = E_syst_idx; iEsysP = E_syst_proton_idx;

            hist_ProtontR = Hist_ProtontR;
            hist_C12Scatter = Hist_C12Scatter;
            hist_O16Deex = Hist_O16Deex;

            SetBinLims(1, Hist_ProtontR->GetXaxis()->GetNbins());
        ...
        void InitAlphaN(const std::string NormPR_name, const std::string NormGS_name, const std::string NormES_name, const std::string E_syst_name,
            ...
            InitAlphaN(Vars->findIdx(NormPR_name), Vars->findIdx(NormGS_name), Vars->findIdx(NormES_name), Esysts->findIdx(E_syst_name),
